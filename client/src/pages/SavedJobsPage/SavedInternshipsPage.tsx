...
const SavedInternshipsPage = () => {
  ...
  const loadSavedInternships = () => {
    // Load saved internship IDs from localStorage
    const savedIds = localStorage.getItem("savedInternships");
    const ids = savedIds ? JSON.parse(savedIds) : [];

    // Filter mock internships to show only saved ones
    const saved = mockInternships.filter((internship) =>
  ...
  useEffect(() => {
    ...
    window.addEventListener("savedInternshipsUpdated", handleStorageUpdate as EventListener);
    return () => window.removeEventListener("savedInternshipsUpdated", handleStorageUpdate as EventListener);
  }, []);

  const handleRemoveSaved = (internshipId: number) => {
    const savedIds = localStorage.getItem("savedInternships");
    const ids = savedIds ? JSON.parse(savedIds) : [];
    const updated = ids.filter((id: number) => id !== internshipId);
    ...
    setSavedInternships(savedInternships.filter((i) => i.id !== internshipId));
  };

  if (loading) {
    return (
      <PortalLayout title="Saved Internships">
        ...
        {savedInternships.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-12 text-center">
            ...
            <p className="text-gray-500">
              You haven't saved any internships yet. Save internships from the
              home page to view them here!
            </p>
          </div>
        ) : (
                ...
                <div className="px-4 py-5 sm:p-6">
                  <div className="flex gap-x-4 justify-between">
                    ...
                    <button
                      ...
                      className="p-1 rounded hover:bg-gray-100 transition-colors text-indigo-600"
